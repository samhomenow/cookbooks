#
# Cookbook Name:: server-hardening
# Recipe:: default
#
...
bash "swap creation" do
	code <<-EOH
	total_mem="`free -gt | perl -ne 'if(/(Mem:)(\s+)([0-9]+)(.*)/) {print $3}'`"
        if [[ ${total_mem} -le 10 ]]
	then
